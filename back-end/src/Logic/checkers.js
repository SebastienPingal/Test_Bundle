function test_json(leJson) {
    let statusCode = [];
    let maxLvlAllowed = 3;
    if (check_json_lvl(leJson) > maxLvlAllowed) statusCode.push(1);
    if (check_json_contain_array(leJson).length != 0) statusCode.push(2);

    if (check_processing_error(leJson) != 0) {
        statusCode.push(3);
        const temp = check_processing_error(leJson);
        statusCode.push(temp.length);
        for (const t in temp) statusCode.push(temp[t]);
    }
    if (check_json_less_than_64_char(leJson) != 0) statusCode.push(4);
        ...
        switch (lArray[t]) {
            ...
            case 3:
                for (let i = 2; i <= lArray[Number(t) + 1] + 1; i++)
                    log_error +=
                        "Processing error at " + lArray[Number(t) + i] + "\n";
                break;
            case 4:
                log_error += "At least one key is too long" + "\n";
                break;
            default:
                log_error += "Erreur inconnue";
        }
    }
    if (log_error == "") console.log("Valid Json");
    else console.log(log_error);
}
